...
export default function NewPagePage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json.error ?? "Erro ao criar página.");
        return;
      }
  ...
  return (
    <div style={{ maxWidth: "640px" }}>
      ...
      <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#0f172a", marginBottom: "8px" }}>Nova página</h1>
      <p style={{ color: "#64748b", marginBottom: "32px" }}>Crie uma nova página e edite com o editor visual de blocos.</p>
      ...
      <div style={{ backgroundColor: "white", border: "1px solid #e2e8f0", borderRadius: "16px", padding: "32px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <div>
            <label htmlFor="title" style={labelStyle}>Título da página *</label>
            <input
              id="title"
              name="title"
              type="text"
          ...
          <div>
            <label htmlFor="slug" style={labelStyle}>
              Slug *{" "}
              <span style={{ color: "#94a3b8", fontWeight: 400 }}>— /s/site/<strong>{slug || "sobre-nos"}</strong></span>
            </label>
            <input
              id="slug"
              name="slug"
            ...
            <p style={{ fontSize: "0.8125rem", color: "#94a3b8", marginTop: "4px" }}>
              Use "/" para a página inicial (home). Apenas letras minúsculas, números e hífens.
            </p>
          </div>

          <div>
            <label htmlFor="template" style={labelStyle}>Template (opcional)</label>
            <select id="template" name="template" style={inputStyle}>
              <option value="">Página em branco</option>
              <option value="landing">Landing Page</option>
              <option value="about">Página Sobre</option>
              <option value="contact">Página Contato</option>
            </select>
          </div>
